%outline:
%(fast) datastructures are pretty essential in general
%fancy datastructures tend to be more complex, making it harder to show all operations on them are correct
%this is why we're trying to make making datastructures and getting parts of correctness easier
%our first step is describing the structure of types inside the language itself
%this only works if the description is sophisticated enough to actually capture the datastructures of interest
%we will be using finger trees as a benchmark, relatively simple in theory, but also using some features like nesting which most descriptions lack
%if you can describe types, you can also describe how to make one type into another, which we encode by ornaments
%these will come into work in the next step
%we note that finger trees, and many interesting datastructures, fit into the framework of calculating datastructures
%but manually performing these calculations is pretty tedious, so we demonstrate that this operation can be implemented as a generic program
