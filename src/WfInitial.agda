...
InitM D .initiality AlgX = p , p!
  ...
  p! (alg→ f sqr) = rec squash₁ (λ s → ∣ Alg→-Path _ _ (funExt (fold≡f s)) ∣₁) sqr
    where -- unfortunately where-modules seem to not handle parameters
...
mainResult1 : (D : Desc) → (AlgX : Algebra (Ḟ D)) → InitAlg (RawḞ D) AlgX → AlgX .Carrier ≃ μ D
mainResult1 D AlgX InitX = InitAlg-Iso (FunḞ D) AlgX (μ-Alg D) InitX (InitM D)



-- mainResult2 : ∀ {D} → (AlgX : Algebra (Ḟ D)) → Wf D (Algebra→Alg AlgX) → InitAlg (RawḞ D) AlgX
-- mainResult2 = {!!}
