...
circle : {D : Desc} → (E : Desc) → ⟦ E ⟧ (μ D) → ℕ
circle (ṿ zero) x = 0
circle (ṿ (suc x₁)) (con x ∷ x₂) = circle {!con x ∷ x₂!} x + circle _ x₂
circle (σ S D) x = suc (circle (D (fst x)) (snd x))

-- x < con x ∷ xs is discounted
-- con x ∷ xs : ⟦ ṿ (suc n) ⟧ (μ D)
