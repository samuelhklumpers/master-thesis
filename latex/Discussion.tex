...
Due to the \AF{remember}-\AF{forget} isomorphism \cite{algorn}, we have that \AF{trieOD}\ \AV{ND} is equivalent to \AV{Σ}\ (\AD{μ} \AV{ND})\ (\AF{itrieOD}\ \AV{ND}), whence \AF{trieOD}\ \AV{ND} is a normal (also referred to as Naperian) functor. This yields traversability of \AF{trieOD}\ \AV{ND}, and consequently \AF{toList}\footnote{Note that the foldable structure we get from the generic \AF{fold} is significantly harder to work with for this purpose.}. 
