...
Due to the \AIC{δ•} constructor \AD{OrnDesc}, \AD{ConOrnDesc}, and \AF{toDesc} become tightly connected\footnote{We left out the variable square for \AIC{δ•}, because it is honestly just too long. If this was included, then the mutual block would also include \AF{ornForget} and its friend \AF{ornErase}.}, so we give the definition as one large mutual block.
...
But let us make the uses of \AD{OrnDesc} more clear by means of examples, where we make use of the variants of some ornaments specialized to binding or ignoring fields\footnote{With analogues for \AIC{Δσ}, (\AIC{Δ})\AIC{δ}, \AIC{δ•}, and like before the non-nested recursive field.}:
