...
\todo[inline, color=red]{Only sig is fine}
...
This can be generalized even further to transport proofs about operations from \bN{} to \bL{}.\todo[inline, color=red]{Explain set?} \todo[inline]{Use an example that doesn't use set.}
...
which infers the definition from the right endpoint of a path using an implicit pair type\todo[inline]{Flip these}
...
We remark \AgdaFunction{Def} is close in concept to refinement types\footnote{À la \href{https://agda.github.io/agda-stdlib/Data.Refinement.html}{Data.Refinement}.}, but importantly, the equality proof is relevant for us, and the value is inferred\todo[inline, color=red]{Deduced?}\todo[inline]{Extracted from the conclusion, rather than requested before the first step?} rather than given. \footnote{Unfortunately, normalizing an application of a \AgdaFunction{defined-by} function also causes a lot of unnecessary wrapping and unwrapping, so \AgdaFunction{Def} is mostly only useful for presentation.} %for now..
