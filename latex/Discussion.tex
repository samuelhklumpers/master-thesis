...
We can define \AF{PathOD} as a generic ornament from a \AD{DescI}\ \AF{Number} to the corresponding finite type, such that \AF{PathOD}\ \AV{ND}\ \AV{n} is equivalent to \AD{Fin}\ (\AF{value}\ \AV{n}). Then, we can show that \AF{itrieOD}\ \AV{ND}\ \AV{n} has a \AF{tabulate}/\AF{lookup} pair for \AF{PathOD}\ \AV{ND}\ \AV{n}, from which it follows that \AF{itrieOD}\ \AV{ND}\ \AV{n}\ \AV{A} is equivalent to \AF{PathOD}\ \AV{ND}\ \AV{n}\ \AV{→}\ \AV{A}, and in consequence \AF{itrieOD}\ \AV{ND} corresponds to \AD{Vec}. From the \AF{Recomputation} lemma it follows that the index \AV{n} of \AF{itrieOD}\ \AV{ND}\ \AV{n} corresponds to applying \AF{ornForget} twice.

Due to the \AF{remember}-\AF{forget} isomorphism \cite{algorn}, we have that \AF{trieOD}\ \AV{ND} is equivalent to \AV{Σ}\ (\AD{μ} \AV{ND})\ (\AF{itrieOD}\ \AV{ND}), whence \AF{trieOD}\ \AV{ND} is a normal functor (also referred to as Traversable). This yields traversability of \AF{trieOD}\ \AV{ND}, and consequently \AF{toList}\footnote{Note that the foldable structure we get from the generic \AF{fold} is significantly harder to work with for this purpose.}. 

We know that the upper square in
% https://q.uiver.app/#q=
\[\begin{tikzcd}
...
\end{tikzcd}\]
commutes, and due to the recomputation lemma, the outer square also commutes. Because \AF{ornForget} from \AF{itrieOD}\ \AV{ND} to \AF{trieOD}\ \AV{ND} is ``epi'' (that is, it covers by ranging over \AV{n}), we find that the lower square also commutes.
...
% bound of Ω(√n) for insertion and deletion if the data structure is uniquely
