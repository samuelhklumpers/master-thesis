...
toℕ-suc (x 2b) = cong
  (λ k → (1 + double k))
  (toℕ-suc x) ∙ cong ℕ.suc (NP.·-suc 2 ⟦ x ⟧) 
\end{code}
...
fromℕ-1+2· (ℕ.suc x) = cong
  (bsuc ∘ bsuc)
  (cong fromℕ (NP.+-suc x (x + ℕ.zero)) ∙ fromℕ-1+2· x)
\end{code}
%<*fromN-2>
...
\begin{code}[hide]
    where
    eq : ∀ x y
       → (1 + 2 · x) + (1 + 2 · y) Eq.≡ 2 + (2 · (x + y))
    eq = NS.solve-∀
    
    solved : fromℕ ((1 + 2 · ⟦ x ⟧) + (1 + 2 · ⟦ y ⟧)) ≡ fromℕ (2 + (2 · (⟦ x ⟧ + ⟦ y ⟧)))
    solved = cong fromℕ (Eq.eqToPath (eq ⟦ x ⟧ ⟦ y ⟧))
plus-def (x 1b) 0b     =
      bsuc (fromℕ (⟦ x ⟧ + (⟦ x ⟧ + 0) + 0))
        ≡⟨ cong (bsuc ∘ fromℕ) (NP.+-zero (double ⟦ x ⟧)) ⟩
      bsuc (fromℕ (⟦ x ⟧ + (⟦ x ⟧ + 0)))
        ≡⟨ fromℕ-1+2· ⟦ x ⟧ ⟩
      fromℕ ⟦ x ⟧ 1b                                         
      ...
      x 1b ∎ use-as-def
plus-def (x 1b) (y 2b) =
  fromℕ (⟦ x 1b ⟧ + ⟦ y 2b ⟧)                      ≡⟨ cong fromℕ (Eq.eqToPath (eq ⟦ x ⟧ ⟦ y ⟧)) ⟩
  bsuc (fromℕ (2 + (double (⟦ x ⟧ + ⟦ y ⟧))))         ≡⟨ cong bsuc (fromℕ-2+2· (⟦ x ⟧ + ⟦ y ⟧)) ⟩
  bsuc (fromℕ (⟦ x ⟧ + ⟦ y ⟧)) 1b ≡⟨ cong (λ z → bsuc z 1b) (by-definition (plus-def x y)) ⟩
  bsuc (defined-by (plus-def x y)) 1b ∎                  use-as-def
    where
    eq : (x y : ℕ) → (1 + 2 · x) + (2 + 2 · y) Eq.≡ 3 + (2 · (x + y))
    eq = NS.solve-∀
plus-def (x 2b) 0b     = (cong fromℕ (NP.+-comm (⟦ x 2b ⟧) _) ∙ by-definition (plus-def 0b (x 2b))) use-as-def
plus-def (x 2b) (y 1b) = (cong fromℕ (NP.+-comm (⟦ x 2b ⟧) _) ∙ by-definition (plus-def (y 1b) (x 2b))) use-as-def
  -- I kind of expected the termination checker to unfold this far enough..
plus-def (x 2b) (y 2b) =
  fromℕ (⟦ x 2b ⟧ + ⟦ y 2b ⟧)                      ≡⟨ cong fromℕ (Eq.eqToPath (eq ⟦ x ⟧ ⟦ y ⟧)) ⟩
  (fromℕ (4 + (double (⟦ x ⟧ + ⟦ y ⟧))))              ≡⟨ cong (bsuc ∘ bsuc) (fromℕ-2+2· (⟦ x ⟧ + ⟦ y ⟧)) ⟩
  bsuc (fromℕ (⟦ x ⟧ + ⟦ y ⟧)) 2b                      ≡⟨ cong (λ z → bsuc z 2b) (by-definition (plus-def x y)) ⟩
  bsuc (defined-by (plus-def x y)) 2b ∎                  use-as-def
    where
    eq : (x y : ℕ) → (2 + 2 · x) + (2 + 2 · y) Eq.≡ 4 + (2 · (x + y))
    eq = NS.solve-∀
\end{code}
