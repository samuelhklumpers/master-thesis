...
data RDesc (I : Set ℓ) (ℓ′ : Level) : Set (lsuc ℓ ⊔ lsuc ℓ′) where
  ṿ   : (is : List I) → RDesc I ℓ′
  σ   : (S : Set ℓ′) (D : S → RDesc I ℓ′) → RDesc I ℓ′

syntax σ S (λ s → D) = σ[ s ∈ S ] D

_σ′_ : ∀ {I : Set ℓ} → Set ℓ′ → RDesc I ℓ′ → RDesc I ℓ′
S σ′ D = σ[ _ ∈ S ] D

infixr 10 _σ′_

Desc : Set ℓ → (ℓ′ : Level) → Set (lsuc ℓ ⊔ lsuc ℓ′)
...
⟦_⟧ : {I : Set ℓ} → RDesc I ℓ′ → (I → Set (ℓ ⊔ ℓ′)) → Set (ℓ ⊔ ℓ′)
⟦ ṿ is  ⟧ X = Ṗ is X
⟦ σ S D ⟧ X = Σ[ s ∈ S ] ⟦ D s ⟧ X

Ḟ : {I : Set ℓ} → Desc I ℓ′ → (I → Set (ℓ ⊔ ℓ′)) → (I → Set (ℓ ⊔ ℓ′))
Ḟ D X i = ⟦ D i ⟧ X

_⇉_ : {I : Set ℓ} → (I → Set ℓ′) → (I → Set ℓ″) → Set (ℓ ⊔ ℓ′ ⊔ ℓ″)
X ⇉ Y = ∀ {i} → X i → Y i

data μ {I : Set ℓ} (D : Desc I ℓ′) : I → Set (ℓ ⊔ ℓ′) where
  con : Ḟ D (μ D) ⇉ μ D

-- without-K kills the termination here
mutual
  {-# TERMINATING #-}
  fold : {I : Set} {D : Desc I ℓ} {X : I → Set ℓ} → Ḟ D X ⇉ X → μ D ⇉ X
  fold {D = D} f {i} (con ds) = f (mapFold (D i) f ds)

  mapFold : {I : Set} {D : Desc I ℓ} (D' : RDesc I ℓ) → {X : I → Set ℓ} → (Ḟ D X ⇉ X) → ⟦ D' ⟧ (μ D) → ⟦ D' ⟧ X
  mapFold (ṿ is)   f ds         = mapFold-Ṗ f is ds
  mapFold (σ S D') f (s , ds)   = s , mapFold (D' s) f ds

  mapFold-Ṗ : {I : Set} {D : Desc I ℓ} {X : I → Set ℓ} → (Ḟ D X ⇉ X) → (is : List I) → Ṗ is (μ D) → Ṗ is X
  mapFold-Ṗ f []       _        = _
...
data ROrn {I : Set ℓ} {J : Set ℓ′} {ℓ″} (e : J → I) : RDesc I ℓ″ → RDesc J ℓ″ → Set (lsuc ℓ ⊔ lsuc ℓ′ ⊔ lsuc ℓ″) where
  ṿ   : {js : List J} {is : List I} (eqs : Ė e js is) → ROrn e (ṿ is) (ṿ js)
  σ   : (S : Set ℓ″) {D : S → RDesc I ℓ″} {E : S → RDesc J ℓ″} (O : (s : S) → ROrn e (D s) (E s)) → ROrn e (σ S D) (σ S E)
  Δ   : (T : Set ℓ″) {D : RDesc I ℓ″} {E : T → RDesc J ℓ″} (O : (t : T) → ROrn e D (E t)) → ROrn e D (σ T E)
  ∇   : {S : Set ℓ″} (s : S) {D : S → RDesc I ℓ″} {E : RDesc J ℓ″} (O : ROrn e (D s) E) → ROrn e (σ S D) E

syntax Δ T (λ t → O) = Δ[ t ∈ T ] O

Orn : {I : Set ℓ} {J : Set ℓ′} (e : J → I) (D : Desc I ℓ″) (E : Desc J ℓ″) → Set (lsuc ℓ ⊔ lsuc ℓ′ ⊔ lsuc ℓ″)
Orn {I = I} e D E = {i : I} (j : Inv e i) → ROrn e (D i) (E (und j))

erase : {I J : Set} {e : J → I} {D : RDesc I ℓ} {E : RDesc J ℓ} → ROrn e D E → {X : I → Set ℓ} → ⟦ E ⟧ (X ∘ e) → ⟦ D ⟧ X
erase (ṿ []          ) _        = _
...
ornAlg : {I J : Set} {e : J → I} {D : Desc I ℓ} {E : Desc J ℓ} (O : Orn e D E) → Ḟ E (μ D ∘ e) ⇉ (μ D ∘ e)
ornAlg O {j} = con ∘ erase (O (ok j))

ornForget : {I J : Set} {e : J → I} {D : Desc I ℓ} {E : Desc J ℓ} (O : Orn e D E) → μ E ⇉ (μ D ∘ e)
...
data ROrnDesc {I : Set ℓ} (J : Set ℓ′) {ℓ″} (e : J → I) : RDesc I ℓ″ → Set (lsuc ℓ ⊔ ℓ′ ⊔ lsuc ℓ″) where
  ṿ   : {is : List I} (js : Ṗ is (Inv e)) → ROrnDesc J e (ṿ is)
  σ   : (S : Set ℓ″) {D : S → RDesc I ℓ″} (O : (s : S) → ROrnDesc J e (D s)) → ROrnDesc J e (σ S D)
  Δ   : (T : Set ℓ″) {D : RDesc I ℓ″} (O : T → ROrnDesc J e D) → ROrnDesc J e D
  ∇   : {S : Set ℓ″} (s : S) {D : S → RDesc I ℓ″} (O : ROrnDesc J e (D s)) → ROrnDesc J e (σ S D)

OrnDesc : {I : Set ℓ} (J : Set ℓ′) (e : J → I) (D : Desc I ℓ″) → Set (lsuc ℓ ⊔ ℓ′ ⊔ lsuc ℓ″)
...
toRDesc : {I : Set ℓ} {J : Set ℓ′} {e : J → I} {D : RDesc I ℓ″} → ROrnDesc J e D → RDesc J ℓ″
toRDesc (ṿ {is} js) = ṿ (und-Ṗ is js)
toRDesc (σ S O)     = σ[ s ∈ S ] toRDesc (O s)
toRDesc (Δ T O)     = σ[ t ∈ T ] toRDesc (O t)
toRDesc (∇ s O)     = toRDesc O

⌊_⌋ : {I : Set ℓ} {J : Set ℓ′} {e : J → I} {D : Desc I ℓ″} → OrnDesc J e D → Desc J ℓ″
⌊ O ⌋ j = toRDesc (O (ok j))
...
toROrn : {I : Set ℓ} {J : Set ℓ′} {e : J → I} {D : RDesc I ℓ″} → (O : ROrnDesc J e D) → ROrn e D (toRDesc O)
toROrn (ṿ js)  = ṿ (to≡-Ṗ _ js)
toROrn (σ S O) = σ[ s ∈ S ] toROrn (O s)
toROrn (Δ T O) = Δ[ t ∈ T ] toROrn (O t)
toROrn (∇ s O) = ∇ s (toROrn O)

⌈_⌉ : {I : Set ℓ} {J : Set ℓ′} {e : J → I} {D : Desc I ℓ″} → (O : OrnDesc J e D) → Orn e D ⌊ O ⌋
