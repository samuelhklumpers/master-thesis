...
In this section, we will demonstrate how we can use the ornamental descriptions to generically compute numerical representations. More precisely, we will define \AF{TreeOD}, which sends a number system to the corresponding type of (nested) full trees over it.
...
The \AF{PhalanxND} in the last constructor gets replaced with \AF{DigitOD} via \AF{O∙δ+}, and the recursive field gets replaced by a recursive field nesting over vectors of length. Again, this is equivalent to \AF{FingerOD}, wrapping values in length one vectors and inserting empty vectors.
